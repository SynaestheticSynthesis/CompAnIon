...
class StoryMode extends StatefulWidget {
  final String? lastEmotion; // Pass last emotion for adaptive branching
  const StoryMode({super.key, this.lastEmotion});
...
class _StoryModeState extends State<StoryMode> {
  ...
  void initState() {
    ...
    _currentNodeId = widget.lastEmotion == null
        ? 'start'
        : (widget.lastEmotion!.contains('Sad') ? 'sad_branch' : 'start');
  }
  ...
  Map<String, StoryNode> _buildStory() {
    return {
      ...
      'sad_branch': StoryNode(
        id: 'sad_branch',
        prompt: "Φαίνεται πως νιώθεις λύπη. Θέλεις να μοιραστείς τι σε βαραίνει ή να συνεχίσεις τη διαδρομή;",
        choices: [
          StoryChoice(text: "Μοιράζομαι", nextNodeId: 'share'),
          StoryChoice(text: "Συνεχίζω", nextNodeId: 'adventure'),
        ],
      ),
      'adventure': StoryNode(
        id: 'adventure',
